    ...
    if ← allExist buildPaths then
      if overwrite || !(← zipPath.pathExists) then
        let task ← IO.asTask do
          ...
          let existingPaths ← buildPaths.filterM (·.1.pathExists)
          let trace :: args := existingPaths.map (·.1.withoutParent packageDir |>.toString)
            | unreachable!
          discard <| runCmd (← getLeanTar) <|
            #["-C", packageDir.toString, zipPath.toString, trace] ++
...
def unpackCache (hashMap : ModuleHashMap) (force : Bool) : CacheM Unit := do
  ...
  if size > 0 then
    ...
    let config : Array Lean.Json ← hashMap.foldM (init := #[]) fun config mod hash => do
      ...
      let packageDir ← getSrcDir sp mod
      return config.push <| .mkObj [("file", pathStr), ("base", packageDir.toString)]
