...
lemma IsClosed.convexHull_subset_coneHull_isVisible (hs : IsClosed (convexHull ℝ s))
    (hx : x ∉ convexHull ℝ s) :
    convexHull ℝ s ⊆ coneHull ℝ x {y ∈ s | IsVisible ℝ (convexHull ℝ s) x y} := by
  rintro y hy
  obtain ⟨z, hz, hxzy, hxz⟩ := hs.exists_wbtw_isVisible hy x
  rw [mem_coneHull]
  rw [wbtw_def] at hxzy
  rcases hxzy with ⟨t, ⟨ht0, ht1⟩, rfl⟩
  rw [lineMap_apply]
  simp only [vadd_vsub]
  have ht_pos : 0 < t := by
    by_contra ht
    rw [le_antisymm (not_lt.1 ht) ht0] at hxzy
    simp at hxzy
    subst y
    exact hx hy
  refine (ConvexCone.hull ℝ _).smul_mem ht_pos ?_
  rw [← mem_coneHull]
  apply convexHull_subset_coneHull
  exact hxz.mem_convexHull_isVisible hx hz
...
lemma rank_le_card_isVisible (hs : IsClosed (convexHull ℝ s)) (hx : x ∉ convexHull ℝ s) :
  ...
  calc
    Module.rank ℝ (span ℝ (-x +ᵥ s)) ≤
      ...
      gcongr
      exact (subset_convexHull ..).trans <| (hs.convexHull_subset_coneHull_isVisible hx).trans
        (coneHull_subset_affineSpan _ _)
