...
local instance {α : Type*} : Trans (Membership.mem : α → Set α → Prop) (HasSubset.subset : Set α → Set α → Prop) (Membership.mem : α → Set α → Prop) where
  trans h1 h2 := h2 h1
...
lemma IsClosed.convexHull_subset_affineSpan_isVisible (hs : IsClosed (convexHull ℝ s))
  ...
  calc y
    _ ∈ affineSpan ℝ (insert x {z}) :=
      AffineSubspace.right_mem_of_wbtw hxzy (mem_affineSpan_insert_self _ _)
        (mem_affineSpan_insert_of_mem _ rfl) (ne_of_mem_of_not_mem hz hx).symm
    _ ⊆ affineSpan ℝ ({x} ∪ {y ∈ s | IsVisible ℝ (convexHull ℝ s) x y}) := by
      rw [affineSpan_le]
      rw [insert_subset_iff, singleton_subset_iff]
      exact ⟨subset_affineSpan _ _ <| subset_union_left rfl,
             affineSpan_mono _ subset_union_right <| convexHull_subset_affineSpan _ <|
               hxz.mem_convexHull_isVisible hx hz⟩
