...
variable {P : Type*} [Semiring 𝕜] [PartialOrder 𝕜] [AddCommGroup M] [Module 𝕜 M] [AddTorsor M P]

/-- The cone with apex `x` generated by `s`. -/
def coneHull (𝕜 : Type*) (x : P) (s : Set P) : Set P :=
  (· +ᵥ x) '' (ConvexCone.hull 𝕜 ((fun y => y -ᵥ x) '' s) : Set M)

@[simp]
lemma mem_coneHull {x y : P} {s : Set P} :
    y ∈ coneHull 𝕜 x s ↔ y -ᵥ x ∈ ConvexCone.hull 𝕜 ((fun z => z -ᵥ x) '' s) := by
  rw [coneHull, mem_image]
  constructor
  · rintro ⟨v, hv, rfl⟩; simp [hv]
  · intro h; exact ⟨y -ᵥ x, h, by simp⟩

lemma subset_coneHull (x : P) (s : Set P) : s ⊆ coneHull 𝕜 x s := by
  intro y hy
  rw [mem_coneHull]
  apply ConvexCone.subset_hull
  use y, hy, rfl

lemma coneHull_convex (x : P) (s : Set P) : Convex 𝕜 (coneHull 𝕜 x s) := by
  rw [coneHull]
  refine (ConvexCone.convex _).affine_image (AffineEquiv.constVAdd 𝕜 M x).toAffineMap

lemma convexHull_subset_coneHull (x : P) (s : Set P) : convexHull 𝕜 s ⊆ coneHull 𝕜 x s :=
  convexHull_min (subset_coneHull x s) (coneHull_convex x s)

lemma coneHull_subset_affineSpan (x : P) (s : Set P) :
    coneHull 𝕜 x s ⊆ affineSpan 𝕜 ({x} ∪ s) := by
  intro y hy
  rw [mem_coneHull] at hy
  have h_span : (fun z => z -ᵥ x) '' s ⊆ (affineSpan 𝕜 ({x} ∪ s)).direction := by
    rintro v ⟨z, hz, rfl⟩
    exact affineSpan_vsub_mem_direction_iff_mem.2 (mem_union_right _ hz) (mem_union_left _ rfl)
  have h_hull : (ConvexCone.hull 𝕜 ((fun z => z -ᵥ x) '' s) : Set M) ⊆
      (affineSpan 𝕜 ({x} ∪ s)).direction := by
    rw [← Submodule.coe_toConvexCone]
    apply ConvexCone.hull_min h_span
    exact Submodule.toConvexCone_le_toConvexCone.2 le_rfl
  rw [← vsub_vadd y x]
  refine AffineSubspace.vadd_mem_of_mem_direction (h_hull hy) (mem_union_left _ rfl)
